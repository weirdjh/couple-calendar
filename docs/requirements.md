...
- Event title, start time, end time, all-day option, and color/category.
- Simple memo on each event.
- Photo upload on each event.
...
- Simple event detail screen that shows memo, photos, reminders, and linked items.
...
3. User taps add.
4. User enters title, time, memo, photos, and reminder.
5. Event appears on both users' shared calendar.
...
1. User registers a base date such as first meeting date or relationship start date.
2. App computes 100 days, 200 days, 1 year, and future anniversaries.
3. Calendar shows those dates as anniversary markers.
4. User can open the anniversary detail or create a related event.
...
- An event can contain at least one uploaded photo.
...
- Should each user have private events, or is every calendar event shared?
...
- What photo limits should exist per event?
