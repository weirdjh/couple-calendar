...
- Go API server for business logic and domain orchestration.
- Firebase for authentication, data, file storage, and notifications behind the
  API server over time.

Current transition note:

- The app currently still has direct Firebase repository implementations.
- New backend work should move toward `Flutter app -> Go API -> Firebase`.
- See `docs/architecture/api-server-migration.md` for the migration plan.
...
`profileColorValue` may be stored on the user profile or couple membership,
depending on whether colors are global per user or per couple space.
...
`memberColorMap` can be used in MVP to assign stable user A/B colors inside the
couple space.
...
Invite codes should be represented by separate `coupleInvites/{code}` documents
instead of querying every couple by `inviteCode`. This keeps the join flow
separate from the couple's private data and gives us a place to add expiration,
revocation, and invite audit behavior later.
...
- Events may span multiple days, including overnight timed events.
- For all-day events, store a predictable normalized exclusive end range rather
  than relying on a missing time.
- For instant records, set `endAt` to the same value as `startAt` or use a short normalized duration consistently.
- `memo` can be empty.
- `ownership` should be `personal` or `shared`.
- `ownerUserId` is required for personal events and usually equals
  `createdBy`.
- `watcherUserIds` stores users watching a partner-owned personal event.
- `reminders` and `linkedItems` default to empty lists.
- New product flows should not attach photos directly to `CalendarEvent`.
  Photos belong to linked records such as date records and reviews.
- `color` should represent creator/user color by default. If category color is
  added later, keep it secondary to creator identity.
- User-facing labels are derived:
  - current user owns personal event: 내 일정
  - partner owns personal event: 상대 일정
  - shared event: 우리 일정
- Current user can edit/delete personal events they own and shared events.
  Current user cannot edit/delete partner-owned personal events.
- Reminder recipients are derived:
  - personal event: `ownerUserId + watcherUserIds`
  - shared event: all couple members
...
Photos should be modeled under the owning feature record, not under
`CalendarEvent`.
...
- Anniversary records own calendar type, base date, repeat rules, and D-day calculations.
...
CalendarEventRepository
  fetchEvents(coupleId, visibleRange)
...
AnniversaryRepository
  ...
  createAnniversary(coupleId, userId, draft)
  updateAnniversary(coupleId, anniversary)
  deleteAnniversary(coupleId, anniversaryId)

TodoRepository
  fetchTodos(coupleId, userId)
  createCategory(coupleId, userId, draft)
  updateCategory(coupleId, category)
  createItem(coupleId, userId, draft)
  updateItem(coupleId, item)
  createCompletion(coupleId, userId, draft)
  deleteCompletion(coupleId, completionId)

DateRecordRepository
  fetchDateRecords(coupleId, userId)
  createRecord(coupleId, userId, draft)
  updateRecord(coupleId, record)
  linkCalendarEvent(coupleId, recordId, eventId)
  unlinkCalendarEvent(coupleId, recordId, eventId)
  addLinkedItem(coupleId, recordId, linkedItem)
  removeLinkedItem(coupleId, recordId, linkedItem)

ReviewRepository
  fetchReviews(coupleId, userId)
  createReview(coupleId, userId, draft)
  updateReview(coupleId, review)
  deleteReview(coupleId, reviewId)

PhotoRepository
  createPlaceholderPhoto(coupleId, ownerPath, uploadedBy, label)
  uploadRecordPhoto(coupleId, ownerPath, file)
  deletePhoto(coupleId, ownerPath, photoId)
...
The current codebase now uses repository ports for Calendar, Anniversary,
Bucket List, Date Record, Review, and Photo. See
`docs/architecture/ddd-repository-review.md` for
the remaining migration plan.
Use `fetch...` for one-shot repository reads. Add `watch...` methods only when
the implementation returns streams.

## Storage Shape

```text
couples/{coupleId}/dateRecords/{recordId}/photos/{photoId}.jpg
couples/{coupleId}/reviews/{reviewId}/photos/{photoId}.jpg
couples/{coupleId}/conflictRecords/{recordId}/photos/{photoId}.jpg
couples/{coupleId}/reviews/{reviewId}/photos/{photoId}.jpg
```
...
See `docs/product/navigation-and-screens.md` for current product navigation direction.
